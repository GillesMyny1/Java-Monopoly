...
public class GameFrame extends JFrame implements GameView{
    ...
    public GameFrame() throws Exception {
        ...
        int result = JOptionPane.showOptionDialog(null, newPanel, "Monopoly",JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, null, options, null);
        ...
        if (result == JOptionPane.YES_OPTION){
            ...
            while (moveOn == -1) {
                ...
                if (prevGame != null) {
                    try {
                        ...
                        JOptionPane.showMessageDialog(this, "Sorry, we could not find the file related to " + prevGame + "\n" + "Try another file or exit the program.");
            ...
            if (versionNum == JOptionPane.CLOSED_OPTION) {
                JOptionPane.showMessageDialog(this, "Thanks for Playing!");
                System.exit(0);
            }
        ...
        buyTile.addActionListener(new GameController(newGame, State.yes));
        doNotBuyTile.addActionListener(new GameController(newGame, State.no));
        roll.addActionListener(new GameController(newGame, State.roll));
        quit.addActionListener(new GameController(newGame, State.quit));
        checkState.addActionListener(new GameController(newGame, State.state));
        passTurn.addActionListener(new GameController(newGame, State.pass));
        purchaseHouse.addActionListener(new GameController(newGame, State.purchaseHouse));
        prop1.addActionListener(new GameController(newGame, State.buyingHouse1));
        prop2.addActionListener(new GameController(newGame, State.buyingHouse2));
        prop3.addActionListener(new GameController(newGame, State.buyingHouse3));
        saveGame.addActionListener(new GameController(newGame, State.saveGame));
        pass2.addActionListener(new GameController(newGame, State.pass));
        payFine.addActionListener(new GameController(newGame, State.payFine));
    ...
    public void handleMustPayRent(MoneyEvent e){
        ...
        playerMessage.setText("You must pay " + player + ", the owner of this property, " + rent + model.getBoard().getCurrencyInUse() +".\nNow you may check state or pass your turn to the next player\nor check if you can purchase a house.");
        playerPanel.remove(roll);
        playerPanel.remove(quit);
        playerPanel.remove(saveGame);
        playerPanel.remove(payFine);
    ...
    public void handleBankruptPlayer(MoneyEvent e){
        ...
        playerMessage.setText("Sorry "+p.getName()+"! You have gone bankrupt because you have landed on "+owner+"'s property and owe "+rent+" " + model.getBoard().getCurrencyInUse() + ".\nYou have been removed from the game.");
        this.remove(propertyPanel);
        playerPanel.remove(purchaseHouse);
        playerPanel.remove(roll);
        playerPanel.remove(checkState);
    ...
    public void handleUnownedProperty(PropertyEvent e){
        ...
        playerMessage.setText("Would you like to buy " + p + "? It costs " + price + " " + model.getBoard().getCurrencyInUse() + " and you have "
                + funds + " " + model.getBoard().getCurrencyInUse() + ".");
        this.remove(propertyPanel);
    ...
    public void handleCanBuyHouse1(HouseEvent e){
        ...
        playerMessage.setText("You are able to purchase a house/hotel!(5 houses = hotel)\n" +
                "The price of a house for these properties are "+((Property)(p1)).getHousePrice()+ " " + model.getBoard().getCurrencyInUse() + ".\n"+
    ...
    public void handleInJail(PieceEvent e){
        Piece p = e.getCurrentlyActivePiece();
        Game model = (Game) e.getSource();
        playerMessage.setText(p.getName()+ ", you are in Jail.\n You have " + (3 - p.getTurnsInJail()) + " more turns to roll doubles or pay a fine of 50 " + model.getBoard().getCurrencyInUse() + ".\nPlease Roll or pay the fine.");
        this.remove(buyPanel);
        this.remove(propertyPanel);
        this.add(playerPanel, BorderLayout.SOUTH);
        playerPanel.add(roll);
    ...
    public void handleMustPayFine(PieceEvent e){
        Piece p = e.getCurrentlyActivePiece();
        Game model = (Game) e.getSource();
        playerMessage.setText(p.getName()+ ", you have not rolled doubles three times and must a fine of 50 " + model.getBoard().getCurrencyInUse() + ".");
        this.remove(buyPanel);
        this.remove(propertyPanel);
        this.add(playerPanel, BorderLayout.SOUTH);
        playerPanel.remove(roll);
    ...
    public void handleLandOnGo(){
        playerMessage.setText("You are on Go, collect " + Go.getReward()+ " " + newGame.getBoard().getCurrencyInUse() + ". You may pass your turn");
        this.remove(buyPanel);
        this.remove(propertyPanel);
        this.add(playerPanel, BorderLayout.SOUTH);
        playerPanel.remove(roll);
    ...
    public void handlePassedGo() {
        JOptionPane.showMessageDialog(this,"You passed go, collect " + Go.getReward() + " " + newGame.getBoard().getCurrencyInUse());
    }
    ...
    public void handleAITurnEnd(PieceEvent e) {
        ...
        if (piece.isAIPlayer()) {
            ...
            sb.append(piece.getName()).append(" has ").append(piece.getFunds()).append(" ").append(model.getBoard().getCurrencyInUse()).append("\n\n");
